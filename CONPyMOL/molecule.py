...
class Molecule(object):
    ...
    def __init__(self, molID):
        ...
        cmd.iterate("(%s and name ca)" % self.molID, "resid_list.append((resi,resn))", space=self.residue_dict)
        self.seq_length = len(self.residue_dict['resid_list'])

    # Create a function to renumber the residues of this molecule
    def renumber_residues(self):
        self.residue_dict = {'resid_list': []}
        cmd.iterate("(%s and name ca)" % self.molID, "resid_list.append((resi,resn))", space=self.residue_dict)
        renumber_factor = str(1 - int(self.residue_dict['resid_list'][0][0]))
        # Renumber according the factor
        print("Renumbering using factor: %s" % renumber_factor)
        cmd.alter(self.molID, 'resi=int(resi)+%s' % renumber_factor)

    # Create function to load the contacts and return a cmap object
    def load_contacts(self, cmap_fpath, cmap_format="psicov"):
                ...
                for line in cmap_file:
                    ...
                    if len(line) >= 3 and line[res1].isnumeric():
                        ...
                        self.predicted_contacts.append(
                            Contact(molID_A=self.molID, residx_A=int(line[res1]), residx_B=int(line[res2]),
    ...
    def plot_contacts(self, score_threshold=0.5, color_choice="red", label_choice="score", radius_choice=0.2,
        ...
        cmd.center(self.molID)
        cmd.zoom(self.molID)

    # Create a function to group the contacts of this molecule
    def create_groups(self):
        cmd.group("Contacts_%s" % self.molID, "Contact_%s_*" % self.molID)
        cmd.group("Labels_%s" % self.molID, "Label_%s_*" % self.molID)
